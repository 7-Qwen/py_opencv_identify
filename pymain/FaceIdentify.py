...
# 设置返回集合
data_false = {"isSuccess": False, "code": 300}
data_true = {"isSuccess": True, "code": 200}
...
def Face():
    ...
    if request.method == 'POST':
        ...
        if 'file' not in request.files:
            return jsonify(result=data_false, message="file不存在")
        file = request.files['file']
        if file.filename == '':
            return jsonify(result=data_false, message="文件名称为空")
        # 安全校验filename
        if file and allowed_file(file.filename):
            if "modelId" in request.form:
                ...
                if not os.path.exists(trainRealPath):
                    os.remove(fiPath)
                    return jsonify(result=data_false, message="modelId有误")
                recognizer.read(trainRealPath)
                # 获取图片
                ...
                for (x, y, w, h) in faces:
                    ...
                    if score > 60:
                        os.remove(fiPath)
                        return jsonify(result=data_true, modelId=idnum)
                    else:
                        os.remove(fiPath)
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            for imagePath in imagePaths:
                ...
                if "modelId" in request.form:
                    # 检查传递的modelId是否为已存在的id 如果是则重复录入提高准确性如果不是则新建立人脸id
                    id = int(request.form.get("modelId"))
                else:
                    id = np.random.randint(low=11111, high=99999)
                returnId = id
            ...
            print("Training...")
            recognizer.save(trainerPath + str(returnId) + '.yml')
            print("Done.")
            return jsonify(result=data_true, modelId=returnId)
    else:
        return jsonify(result=data_false, message="请求方式不为POST")
